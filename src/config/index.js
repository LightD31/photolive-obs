  ...
  loadConfig() {
    try {
      ...
      this.config = {
        ...
        ftp: {
          enabled: process.env.FTP_ENABLED === 'true' || configData.ftp?.enabled || false,
          port: parseInt(process.env.FTP_PORT, 10) || configData.ftp?.port || 2121,
          username: process.env.FTP_USERNAME || configData.ftp?.username || 'photolive',
          password: process.env.FTP_PASSWORD || configData.ftp?.password || 'photolive',
          pasv_min: configData.ftp?.pasv_min || 1024,
          pasv_max: configData.ftp?.pasv_max || 1048,
        },
